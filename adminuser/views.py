...
class AdminHomepage(TemplateView):
	def get(self,request):
		...
		print("category of admin",counts_by_category)
		# adminList = []
		# for obj in objs:
		# 	print("obj",obj)
...
class AdminSuspiciousCase(TemplateView):
	...
	def post(self,request):
		...
		print(".....")
		obj = AdminFraud.objects.filter(fraud_id=fraud_id).update(fraud_status=value,fraud_identified_date=datetime.now(),admin_flag='1')
		obj1 = Fraud.objects.filter(fraud_id=fraud_id).update(fraud_status=value,fraud_identified_date=datetime.now())
		obj2 = SuperAdminFraud.objects.filter(fraud_id=fraud_id).update(fraud_status=value,fraud_identified_date=datetime.now())
		print("obj1 in post")
